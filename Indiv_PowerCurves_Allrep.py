...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 28 17:11:03 2023
...
output_directory = "/Users/botonddavoti/MasterPython/Power Curves"
...
def process_and_plot_data(df_dictionary, resistance_type, subject, exercise):
    ...
    for df_key_name, df_value_rep in df_dictionary.items():
        ...
        df_value_rep['Power'] = df_value_rep['Barbell force (FP)'] * df_value_rep['Barbell velocity']  # Calculate power
        df_value_rep.drop(
    ...
    if valid_reps:
        ...
        with PdfPages(pdf_path) as pdf:
            for metric in ['Normalized position', 'Power']:
                ...
                for df_key_name, df_value_rep in valid_reps.items():
                    ...
                    ax.set_ylabel(f"{metric} " + ("(W)" if metric == 'Power' else ""))
                    ax.legend()
                pdf.savefig(fig)
                plt.close(fig)
