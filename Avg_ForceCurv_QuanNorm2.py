...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 24 09:52:12 2023
...
# Function to calculate the average quantum force for each exercise
def calculate_quantum_avg_force(exercise_data):
    quantum_forces = []

    # Check if Quantum data exists
    if 'quantum' in exercise_data:
        quantum_data = exercise_data['quantum']

        # Collect force data from all Quantum reps
        for df_key_name, df_value_rep in quantum_data.items():
            quantum_forces.extend(df_value_rep['Barbell force (FP)'].values)

    # Calculate the average force if quantum data is present
    if quantum_forces:
        return np.mean(quantum_forces)
    else:
        return None

# Modified function to calculate the average curve across all reps for each resistance type
def calculate_average_curve(resistance_data, all_positions, quantum_avg_force):
    ...
    # Normalize the average force based on quantum average force
    avg_force = avg_force / quantum_avg_force

    return avg_force

# Generate and save the plots with normalization
def generate_plots(all_data, output_directory, dpi):
    ...
    for exercise_name, exercise_data in all_data.items():
        quantum_avg_force = calculate_quantum_avg_force(exercise_data)
        if quantum_avg_force is not None:
            plt.figure(figsize=(10, 5), dpi=dpi)
            for resistance_type, resistance_data in exercise_data.items():
                avg_force = calculate_average_curve(resistance_data, common_positions, quantum_avg_force)
