        ...
        for metric in ['Barbell position', 'Barbell velocity', 'Barbell force (FP)']:
            ...
            for df_key_name, df_value_rep in df_dictionary.items():
                if metric == 'Barbell position':
                    # Plot Barbell position against Time
                    ax.plot(df_value_rep['timestamp'], df_value_rep[metric], label=df_key_name)
                    ax.set_xlabel('Time (s)')
                    ax.set_ylabel(f"{metric} (m)")
                else:
                    # Plot Barbell velocity and Barbell force against Barbell position
                    ax.plot(df_value_rep['Barbell position'], df_value_rep[metric], label=df_key_name)
                    ax.set_xlabel('Barbell position (m)')
                    ax.set_ylabel(f"{metric} " + ("(m/s)" if metric == 'Barbell velocity' else "(N)"))
