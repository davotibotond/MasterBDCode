...
folder_path = '/Users/botonddavoti/MasterPython/Data/Subject_1/bench/freeweight'
...
for df_key_name, df_value_rep in df_dictionary.items():
    ...
    ax_velocity.plot(df_value_rep['Barbell position'], df_value_rep['Barbell velocity'], label=df_key_name)
    ax_velocity.set_xlabel('Position (m)')
    ax_velocity.set_ylabel('Barbell velocity (m/s)')
    ax_velocity.legend()

    # Plot barbell force (FP)
    ax_force_FP.plot(df_value_rep['Barbell position'], df_value_rep['Barbell force (FP)'], label=df_key_name)
...
# Output directory
output_directory = "/Users/botonddavoti/MasterPython/Test"
