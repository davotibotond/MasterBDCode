...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 23 20:41:40 2023
    ...
    if os.path.isdir(subject_path):
        ...
        for exercise_folder in os.listdir(subject_path):
            ...
            if os.path.isdir(exercise_path):
                ...
                if exercise_data:
                    subject_data[exercise_name] = exercise_data
        if subject_data:
...
def calculate_average_curve(resistance_data, all_positions):
    ...
    # Normalize the average force to have a peak of 1.5 and an average of 1.0
    avg_force = avg_force / np.max(avg_force) * 1.5

    return avg_force

# Generate and save the plots
def generate_plots(all_data, output_directory, dpi):
    for subject_folder, subject_data in all_data.items():
        for exercise_name, exercise_data in subject_data.items():
            ...
            plt.title(f'Average Force Curves - {exercise_name} ({subject_folder})')
            ...
            pdf_filename = f"{subject_folder}_{exercise_name}_average_force_curves.pdf"
